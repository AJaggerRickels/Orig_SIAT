...
from psychopy import event, core, visual, gui, monitors, data #microphone #VMeter #sound
...
# parallel_writer = ctypes.pydll.LoadLibrary('inpout32.dll')
...
def wait_scan_start(win):
    #show "In Preparation" in msg in win, wait for a 'z' after the scanner is prepped, then wait for an 'a' to start the task
    msg = visual.TextStim(win,text="",units='pix',pos=[0,0],color=[win.color[0] * -1, win.color[1] * -1, win.color[2] * -1] ,height=30,wrapWidth=int(1600)) #text color should be opposite of window color
    ...
    msg2.setText('Waiting for cue from scanner to start (a)')
    msg.draw()
    msg2.draw()
    win.flip()
    k = event.waitKeys(maxWait='inf',keyList=['a','escape'],modifiers=False,timeStamped=True,clearEvents=True)
    if event.getKeys == 'escape':
        raise QuitException()
...
def run_instructions(instruct_schedule_file, g):
    ...
    i = 0
    while i < len(slides):
        i = max(i + do_one_slide(slides[i], directory, g), 0) #, isounds[i] do_one_slide may increment or decrement i, depending on whether session_params['right'] or session_params['left'] is pressed--don't let them go back on the first slide
    core.rush(False)
...
def do_one_slide(slide, directory, g): #isound
    #event.clearEvents()
